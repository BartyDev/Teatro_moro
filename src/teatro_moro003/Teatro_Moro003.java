...
//Mi arrayList para el resumen de compra
        ArrayList<String> resumenCompras = new ArrayList<>();
        ...
        for (;;) {
            ...
            System.out.println("4).Resumen de compra");
...
// **Opción de confirmación**
                ...
                while (edad < 1 || edad > 120) {
                    System.out.println("¡Se aplicará el descuento si cumple las condiciones!");
                    ...
                    // **Guardar el resumen en el ArrayList**  
                    String detallesCompra = "Tipo de entrada: " + zonasCompradas[totalFinal - 1]
                            + "\nUbicación: Fila " + fila + ", Asiento " + asientosComprados[totalFinal - 1]
                            + "\nDescuento aplicado: " + descuentosAplicados[totalFinal - 1] + "%"
                            + "\nCosto final: $" + String.format("%.0f", preciosFinales[totalFinal - 1]);
                    resumenCompras.add(detallesCompra); // Guardar en el ArrayList  
                }
                ...
                if (seguir == 1) {
                    ...
                    if (entradaEliminar >= 0 && entradaEliminar < totalFinal) {
                        ...
                        switch (zonaEliminada) {
                            ...
                            case "Palcos" ->
                                zonaArray = palcos;
                            case "Platea Alta" ->
                                zonaArray = plateaAlta;
                            case "Platea Baja" ->
                                zonaArray = plateaBaja;
                            default -> {
                    ...
                    if (entradaModificar >= 0 && entradaModificar < totalFinal) {
                        ...
                        switch (zonaAnterior) {
                            case "VIP" ->
                                zonaAnteriorArray = vip;
                            case "Palcos" ->
                                zonaAnteriorArray = palcos;
                            case "Platea Alta" ->
                                zonaAnteriorArray = plateaAlta;
                            case "Platea Baja" ->
                                zonaAnteriorArray = plateaBaja;
                            default -> {
                                System.out.println("❌ Zona inválida.");
                                continue;
                            }
                ...
                if (resumenCompras.isEmpty()) {
                    System.out.println("No hay compras registradas.");
                } else {
                    for (int i = 0; i < resumenCompras.size(); i++) {
                        System.out.println("Compra #" + (i + 1) + ":");
                        System.out.println(resumenCompras.get(i));
                        System.out.println("------------------------------------------------");
                    }
                }
            } else if (opcion == 5) {
                ...
                for (int i = 0; i < asientos; i++) {
                    if (vip[i].equals(ocupado)) {
                        totalVendidas++;
                    }
                    if (palcos[i].equals(ocupado)) {
                        totalVendidas++;
                    }
                    if (plateaAlta[i].equals(ocupado)) {
                        totalVendidas++;
                    }
                    if (plateaBaja[i].equals(ocupado)) {
                        totalVendidas++;
                    }
                }
                System.out.println("................................");
